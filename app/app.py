...
app.register_blueprint(
    image_comments_bp, url_prefix="/gallery/<gallery_id>/images/<image_id>/comments"
)
...
def ensure_default_session():
    ...
    if session["role"] != "guest":
        if "last_check" in session:
            if datetime.utcnow().replace(tzinfo=timezone.utc) - session[
                "last_check"
            ...
            session["last_check"] = datetime.fromtimestamp(0, timezone.utc).replace(
                tzinfo=timezone.utc
            )
...
if __name__ == "__main__":
    ...
    app.run(
        debug=True,
        host=os.getenv("FALSK_HOST", "0.0.0.0"),
        port=os.getenv("FALSK_PORT", "5000"),
    )
